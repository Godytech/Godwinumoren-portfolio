...
export const EducationSection = () => {
  const { items: education } = useFirestoreCollection<EducationItem>("education", initialPortfolioData.education);
      ...
      <div>
        ...
        >
          <div className="flex items-center gap-4 mb-3">
            <div className="w-12 h-12 rounded-2xl bg-primary/10 border border-primary/25 flex items-center justify-center text-primary shadow-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {education.map((edu, i) => {
            const isFirst = i % 2 === 0;
            const DegreeIcon = isFirst ? GraduationCap : BookOpen;
            const BadgeIcon = isFirst ? Award : Sparkles;
                  ...
                  <div className="flex flex-col h-full justify-between gap-6">
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-4 mb-6">
                        <div className="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/25 text-primary flex items-center justify-center shadow-xs">
                          <DegreeIcon className="w-7 h-7 text-primary" />
                        </div>
                        {edu.badge && (
                          <span className={`px-3.5 py-1.5 rounded-full border text-xs font-extrabold flex items-center gap-1.5 shadow-xs ${badgeColor}`}>
